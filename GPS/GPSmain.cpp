...
int main()
{
	...
	while (1)
	{
		...
		GPSTimeStamp = (double)Counter / (double)Frequency * 1000; //do calculation in double form, 1*1000 ms = 1s
		Console::WriteLine("GPS time stamp :{0,12:F3} {1,12:X3}", GPSTimeStamp, Shutdown); //{0,12:F3} for format manuplation
		Thread::Sleep(25);
		if (Shutdown)
			break;
		if (_kbhit())
